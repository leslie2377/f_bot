...
요금제 결과 표시 규칙 (매우 중요):
- 요금제를 2개 이상 안내할 때는 반드시 마크다운 테이블로 표시하세요.
- 테이블 형식:
| 요금제 | 통신망 | 판매가 | 데이터 | 통화 | 문자 | 상세 |
|--------|--------|--------|--------|------|------|------|
| 요금제명 | SKT | **월 X,XXX원** | 10GB | 300분 | 기본제공 | [보기](URL) |
- 할인 요금제는 판매가 아래에 "(N개월 후 XX,XXX원)" 표기
...
- 고객이 사용량을 알려주면 조건에 맞는 요금제를 테이블로 추천하세요.
- 이미 대화 히스토리에 사용량 정보가 있으면 다시 묻지 말고 바로 추천하세요.
...
function filterProductDocs(docs, cond) {
  ...
  return docs.filter(d => {
    ...
    // 가격 필터
    if (cond.maxPrice && fee > cond.maxPrice) return false;
    ...
    // 통화 필터
    if (cond.voiceMin) {
      const voiceNum = parseInt(voiceStr) || 0;
      const isVoiceUnlimited = voiceStr.includes('무제한') || voiceStr.includes('기본제공');
...
async function smartRetrieve(question, k = 8) {
  ...
  const allDocs = await vectorStore.similaritySearch(searchQuery, k * 3);

  // 조건 필터링 적용
  const filtered = usageCond ? filterProductDocs(allDocs, usageCond) : allDocs;
  ...
  if (usageCond) {
    ...
    const others = filtered.filter(d => d.metadata.type !== 'product');
    return [...products.slice(0, 6), ...others.slice(0, 2)].slice(0, k);
  }
